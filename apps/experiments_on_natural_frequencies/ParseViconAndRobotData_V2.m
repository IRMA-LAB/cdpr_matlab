...
for i=40:l
   ...
   if (i<37)
        ...
        n_f = 2;
   elseif(i<49)
        options = optimoptions('lsqnonlin','SpecifyObjectiveGradient',true,'MaxIterations',2000,'MaxFunctionEvaluations',2000,'OptimalityTolerance',1e-8,'FunctionTolerance',1e-8);
        n_f = 3;
   else
        options = optimoptions('lsqnonlin','SpecifyObjectiveGradient',true,'MaxIterations',20000,'MaxFunctionEvaluations',20000,'OptimalityTolerance',1e-8,'FunctionTolerance',1e-8);
        n_f = 4;
   end
    sol0 = [];
    f_val0 = [];
...
%     lb_single =[-30.*ones(15,1);0;0;0];
%     ub_single = [30.*ones(15,1);1;30;2*pi];
%     lb = []; ub = []; v0 = [];
%     for ii=1:n_f
%        lb = [lb ;lb_single]; 
%        ub = [ub ;ub_single];
...
%     end
%     problem = createOptimProblem('lsqnonlin',...
%     'objective',@(x)IndetifyF_V2_J(x,data_analysis,n_f,n_points,n_coordinates),'lb',lb,'ub',ub,...
%     'x0',v0,'options',options);   
%     [x,ff] = run(ms,problem,10);
%     freq = [];
%     n_var = n_points*n_coordinates+3;
%     for k=1:n_f
%         freq = [freq;x(k*n_var-1)];
%     end
%     save([dest_dir '/' dest_file_name_f num2str(i) '.mat'], 'freq');
%     figg = ComputePlot_V2(x,data_analysis,n_f,n_points,n_coordinates);
%     savefig(figg,[dest_dir '/' dest_file_name_fig num2str(i) '.fig']);
    clear points
    clear data_analysis
    clear figg
