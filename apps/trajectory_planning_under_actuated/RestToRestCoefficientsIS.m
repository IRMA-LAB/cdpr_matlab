function [output] = RestToRestCoefficientsIS(cdpr_p,cdpr_v,...
...
for i = 1:sim_data.pNumber-1
  ...
  tt = linspace(0,sim_data.T*0.4,7);
  coefficientGuess = [tt(2:end)';1];
  sim_data.motion_law_function = @ShapedActuatedDofs;
%   A = zeros(4,6);
%   b = zeros(7,1); b(end) = sim_data.T;
%   for j=1:3
%      A(j,3+j) = -1;
%      A(j+1,3+j) = 1;
%   end
  tic
  [sim_data.coeff(:,i),fval,exitFlag1,Foutput]  = fmincon(@(k)0,coefficientGuess,[],[],[],[],zeros(6,1),sim_data.T.*ones(6,1),@(k)FindOptimalCoefficientsfminconConstrIS(k,cdpr_p,cdpr_v,...
