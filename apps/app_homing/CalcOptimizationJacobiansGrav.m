function [J_sl,J_q,J_g] = CalcOptimizationJacobiansGrav(cdpr_v,Ja,Ju,mg,Hg)
...
for i=1:n
  ...
  for j=1:m
      Arho(:,j) = Anti(dadql(:,j))*cdpr_v.cable(i).vers_rho;
  end
  dJTo(:,i,:) = Anti(cdpr_v.cable(i).pos_PA_glob)*drhodq+Arho;
end
...
dfdg = [-Anti(mg)*Hg;-Anti(cdpr_v.platform.pos_PG_glob)*Anti(mg)*Hg];
dWa(:,:) = dfdq(1:n,:);
dWu(:,:) = dfdq(n+1:end,:);
dfadg(:,:) = dfdg(1:n,:);
dfudg(:,:) = dfdg(n+1:end,:);
...
J_g = Ju'*linsolve(Ja',dfadg)-dfudg;
