function [vector,matrix] = HomingOptimizationFunctionTau3(cdpr_p,record,dln,ds,v)
...
for j = 1:dim    
  pose = v(3*cdpr_p.n_cables+cdpr_p.pose_dim*(j-1)+1:3*cdpr_p.n_cables+j*cdpr_p.pose_dim,1);
  ...
  matrix(equationsXstage*(j-1)+1+cdpr_p.n_cables:equationsXstage*(j-1)+2*cdpr_p.n_cables,1+2*cdpr_p.n_cables:3*cdpr_p.n_cables) = -eye(cdpr_p.n_cables);
  matrix(equationsXstage*(j-1)+1:equationsXstage*(j-1)+2*cdpr_p.n_cables,3*cdpr_p.n_cables+cdpr_p.pose_dim*(j-1)+1:3*cdpr_p.n_cables+j*cdpr_p.pose_dim) = J_sl;
  matrix(equationsXstage*(j-1)+2*cdpr_p.n_cables+1:equationsXstage*j,3*cdpr_p.n_cables+cdpr_p.pose_dim*(j-1)+1:3*cdpr_p.n_cables+j*cdpr_p.pose_dim) = J_q;
  if (j==1)
