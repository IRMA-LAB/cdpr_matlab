...
incr = pi/4;
 for x = 0:0.4:2.6
     for y = -1.3:0.4:1.7
         for z=-2.6:0.2:-1
            for aa = 0:incr:2*pi-incr
                 for bb = 0:incr:2*pi-incr
                      for cc = 0:incr:2*pi-incr
            ii = ii + 1;
           solution_est = [x;y;z;aa;bb;cc];
          ...
          if (~isreal(solution_calc(:,ii)) || (any(isnan(solution_calc(:,ii)))))
             solution_calc(:,ii) = solution_calc(:,1); 
          end
            if (ii == 1)
              count = count +1;
              ...
              isNotStable2 = AssertStability33BruteForce(cdpr_parameters,solution_calc(:,ii),cdpr_variables.platform.pos_OG_glob,diffl0,record,utilities);
              ...
              if (exitflag(ii)>0 )
                ...
                if (flag == c)
                  ...
                  diffs0 = [diffs0 s0];
                  record.SetFrame(cdpr_variables,cdpr_parameters);
                  isNotStable2 = [isNotStable2 AssertStability33BruteForce(cdpr_parameters,solution_calc(:,ii),cdpr_variables.platform.pos_OG_glob,l0,record,utilities)];
