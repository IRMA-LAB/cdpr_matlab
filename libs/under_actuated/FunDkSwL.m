function [vector, matrix] = FunDkSwL(cdpr_p, complete_length, swivel_angles,...
  variables, varargin)

cdpr_v = CdprVar(cdpr_p.n_cables);
sw_constraint = zeros(cdpr_p.n_cables, 1);
[cdpr_v, l_constraint] = CalcKinZeroOrdConstr(variables(1:3), variables(4:end),...
  complete_length, cdpr_p, cdpr_v);
...
vector = [l_constraint; sw_constraint];
matrix = [l_jacobian; sw_jacobian];
