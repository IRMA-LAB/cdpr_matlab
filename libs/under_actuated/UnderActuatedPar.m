classdef UnderActuatedPar
  ...
  methods
    ...
    function obj = UnderActuatedPar(n, n_p, mask)
      obj.n_cables = n;
      obj.pose_dim = n_p;
      obj.permutation_matrix = zeros(n_p);

      actutuated_idx = find(mask);
      unactutuated_idx = find(~mask);
      for i = 1:n
        obj.permutation_matrix(i, actutuated_idx(i)) = 1;
      end
      for i = 1:(n_p - n)
        obj.permutation_matrix(n + i, unactutuated_idx(i)) = 1;
      end
    end
