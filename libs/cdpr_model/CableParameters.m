classdef CableParameters
  ...
  %
  properties
    pos_PA_loc % a vector (size[3,1], [m]), containing the components of the distal anchor point A, projected in the local frame.
    pos_OD_glob % a vector (size[3,1], [m]), containing the components of the proximal anchor point D, projected in the fixed frame.
    vers_i %
    vers_j %
    vers_k %
    rot_mat %
    swivel_pulley_r % the swivel pulley radius.
    swivel_pulley_transmission_ratio % the swivel pulley-encoder transmission ratio.
    l0 % the initial cable length.
    motor_transmission_ratio % the motor-cable transmission ratio.
  end
